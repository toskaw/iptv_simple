...
} elseif (isset($_POST['url-input'])) {
    $m3u8_url = urldecode($_POST['url-input']);
    $ret = get_playlist($m3u8_url);
...
function get_playlist($url) {
    ...
    // playlist
    file_put_contents("custom.m3u8", $playlist);
    
    $current = empty($_SERVER['HTTPS']) ? 'http://' : 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    $m3u8Url = preg_replace('/proxy.php/', 'custom.m3u8', $current);
    $response = array(
        'url' => $m3u8Url 
    );
    return $response;
